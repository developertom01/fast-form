...
async def signup(request: Request, origin:str =Query("web"),):
...
async def submit_form(request:Request,response:Response, origin:str =Query("web"), email= Form(default=""), name = Form(default=""), password= Form(default=""), conn:Connection =  Depends(get_db)):
    ...
    try:
        await conn.execute("INSERT INTO users (id,name,email, password) VALUES (?, ?, ?, ?)",(user_id, name, email, hashed_password ))
        await conn.commit()
